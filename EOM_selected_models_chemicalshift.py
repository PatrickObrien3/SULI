...
for k in range(2,3):
    ...
    pdb,freq=np.loadtxt(filename+str(k)+'/Data/GA00'+str(run)+'/best_curve00'+str(run)+'.txt',skiprows=2,usecols=(0,1),dtype=int, unpack=True)
    os.mkdir(filename+str(k)+'/Data/Selected00'+str(run))
    for i in pdb:
        os.system('ln -s  '+filename+str(k)+'/Data/'+str(i)+'x.pdb   '+filename+str(k)+'/Data/Selected00'+str(run)+'/')
        os.chdir(filename+str(k)+'/Data/Selected00'+str(run)+'/')
    for i in pdb:
